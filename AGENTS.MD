...
The next (recording) subagent should open playwright (using MCP). NEVER run `npx playwright test` to start the playwright. ALWAYS use playwright via MCP. Follow the steps and navigate as defined in the `PATH.json` file. The audio part will be done by another subagent. Before starting the playwright, start a node server by running `node click-feedback-extension/record-server.js` in background. This is required for recording the browser actions. Start the playwright in the headed mode. Immediately after starting the playwright, Press "Alt+Shift+D" shortcut to start recording the browser. This is the MOST IMPORTANT STEP, if you miss this recording will not be generated. There will not be any visual indicator for recording in-progress, so don't look out for one. After all the steps are done, close the playwright MCP browser. This will create the recording video file at `recordings/` folder in the project root with the name `recording-{timestamp}.webm`.Every time a step is started, the current timestamp (`echo $(($(date +%s%N)/1000000))` in terminal) should be found and updated in each step as "timestamp" field in `PATH.json` file and saved immediately. This will be used in the future while attaching audio. 
...
The next (composer) subagent should pick the video file (file with latest timestamp in `recordings/` folder) and all the generated audio files and merge them together. Remember each step in `PATH.json` will contain the timestamp that the step was started. So you should find difference in timestamps between current and previous step, and each audio should be placed at their respective timing gap while attaching to the video file. The audio files has to be merged in a way that the respective audio should be placed at the respective section of the video. The actions in the video and audio should be in sync. The timestamp difference calculation will ensure that. The subagent can use the `ffmpeg` tool, which is already installed in this machine. Here's an example command. 
